...
for (column in names(train %>% select(-dropout))) {
  print(column)
  
  permuted = test
  permuted[[column]] = sample(permuted[[column]])
  predicted_scores = predict(log.reg.fit2, permuted, type = "response")
  
  metrics = get_all_metrics(predicted_scores, true_labels)
  scores = c(scores, metrics$F2score)
}
pfi = data.frame(predictor=names(train %>% select(-dropout)), score=scores)
pfi[order(pfi$score),]
```
